...
final class CustomConcurrentHashMap {
	...
	static final class Builder {
		 ...
		 * @param <E>
		 *            the type of internal entry to be stored in the returned
		 *            map
	 ...
	 *            the type of values to be stored in the returned map
	 * @param <E>
	 *            internal entry type, not directly exposed to clients in map
	 *            views
	 */
	public interface Strategy<K, V, E> {
		 ...
		 * @param next
		 *            entry (used when implementing a hash bucket as a linked
		 ...
		 * 
		 * @param key
		 *            for {@code original} as well as the returned entry.
		 *            Explicitly provided here to prevent reclamation of the key
		 *            at an inopportune time in implementations that don't
		 *            otherwise keep a strong reference to the key.
		 * @param original
		 *            entry from which the value and other
	...
	static class Impl<K, V, E> extends AbstractMap<K, V> implements
				...
				for (int oldIndex = 0; oldIndex < oldCapacity; oldIndex++) {
					...
					if (head != null) {
						...
						if (next == null) {
							...
							for (E last = next; last != null; last = s
									.getNext(last)) {
				...
				try {
					...
					for (E e = first; e != null; e = s.getNext(e)) {
						...
						if (s.getHash(e) == hash && entryKey != null
							...
							V entryValue = strategy.getValue(e);
							if (value == entryValue
									|| (value != null && entryValue != null && s
											.equalValues(entryValue, value))) {
								// All entries following removed node can stay
								// in list, but all preceding ones need to be
								// cloned.
								++modCount;
		 ...
		 * of the hash table, and so is much slower than method
		 * {@code containsKey}.
		 * 
		 * @param value
		 ...
		 * from this map, via the {@code Iterator.remove},
		...
		abstract class HashIterator {
			...
			boolean nextInChain() {
				Strategy<K, V, E> s = Impl.this.strategy;
				if (nextEntry != null) {
					for (nextEntry = s.getNext(nextEntry); nextEntry != null; nextEntry = s
							.getNext(nextEntry)) {
		...
		private void readObject(java.io.ObjectInputStream in)
			...
			try {
				...
				Strategy<K, V, E> strategy = (Strategy<K, V, E>) in
						.readObject();
				...
				if (entry == null) {
					...
					try {
						...
						if (entry == null) {
							...
							E first = table.get(index);
							++segment.modCount;
							entry = computingStrategy
									.newEntry(key, hash, first);
					...
					if (created) {
						...
						try {
							V value = computingStrategy.compute(key, entry,
									computer);
							if (value == null) {
								throw new NullPointerException(
	 ...
	 */
	static class SimpleStrategy<K, V> implements
			Strategy<K, V, SimpleInternalEntry<K, V>> {
	...
	static class SimpleInternalEntry<K, V> {
		...
		SimpleInternalEntry(K key, int hash, V value,
				SimpleInternalEntry<K, V> next) {
			this.key = key;
			this.hash = hash;
			this.value = value;
