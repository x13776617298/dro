...
public class BetterHttp {
	...
	public static void enableResponseCache(Context context,
			int initialCapacity, long expirationInMinutes,
			int maxConcurrentThreads, int diskCacheStorageDevice) {
		enableResponseCache(initialCapacity, expirationInMinutes,
	...
	private static void setCmwapProxy() {

		try {

			HttpParams httpParams = httpClient.getParams();
			Object s = httpParams.getParameter(ConnRoutePNames.DEFAULT_PROXY);
			if (s instanceof HttpHost) {
				HttpHost host = (HttpHost) s;
				if (!"10.0.0.172".equalsIgnoreCase(host.getHostName())) {
	...
	private static boolean isWap() {
		...
		Log.i("BabytreeTag", nwInfo.toString());
		String extraInfo = nwInfo.getExtraInfo();
		if(extraInfo==null){
			return false;
		}
		HttpParams httpParams = httpClient.getParams();
		Object s = httpParams.getParameter(ConnRoutePNames.DEFAULT_PROXY);
		String hostName = "10.0.0.172";
		Log.i("BabytreeTag", extraInfo);
		if ("cmwap".equals(extraInfo)) { // 10.0.0.172
			setwapProxy(hostName);
			return true;
		}

		if ("ctwap".equals(extraInfo)) { // 10.0.0.200
			hostName = "10.0.0.200";
			setwapProxy(hostName);
			return true;
		}

		if ("3gwap".equals(extraInfo)) { // 10.0.0.172
			setwapProxy(hostName);
			return true;
		}

		if ("uniwap".equals(extraInfo)) { // 10.0.0.172
			setwapProxy(hostName);
			return true;
		}
	...
	private static void setwapProxy(String hostName) {
